...
"""A binary to train BBBC006 using multiple GPUs with synchronous updates."""
...
def train():
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        num_batches_per_epoch = (bbbc006.NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN /
                                 FLAGS.batch_size)
        decay_steps = int(num_batches_per_epoch * bbbc006.NUM_EPOCHS_PER_DECAY)

        # Decay the learning rate exponentially based on the number of steps.
        lr = tf.train.exponential_decay(bbbc006.INITIAL_LEARNING_RATE,
                                        global_step,
                                        decay_steps,
                                        bbbc006.LEARNING_RATE_DECAY_FACTOR,
                                        staircase=True)
        ...
        variable_averages = tf.train.ExponentialMovingAverage(
            bbbc006.MOVING_AVERAGE_DECAY, global_step)
